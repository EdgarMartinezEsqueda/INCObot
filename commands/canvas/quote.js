...
async function createBackground() {
    ...
    const ctx = canvas.getContext("2d");
    const imagePath = path.join(__dirname, "../../assets/image/bg.jpg");

    const image = await loadImage(imagePath); // Cargar la imagen del background

    ctx.drawImage(image, -10, -10, canvas.width + 20, canvas.height + 20); // Dibujar la imagen cubriendo todo el canvas con un pequeño margen para evitar bordes
...
function drawText(text, author, canvasWidth = 500, canvasHeight = 350) {
    ...
    const totalHeight =
        lineHeight * Math.ceil(ctx.measureText(text).width / maxWidth);
...
async function drawCombinedCanvas(message, cita, imagen, autor) {
    ...
    combinedCtx.drawImage(
        rectangleCanvas,
        combinedCanvas.width / 2 - 300,
        combinedCanvas.height / 2 - imageCanvas.height,
    );
    combinedCtx.drawImage(
        imageCanvas,
        combinedCanvas.width / 2 - imageCanvas.width / 2,
        50,
    );
    combinedCtx.drawImage(
        textoCanvas,
        combinedCanvas.width / 4 - 40,
        combinedCanvas.height / 2 - 50,
    );
...
function drawMinimalistText(
            ...
            if (wordMetrics.width > maxWidth) {
                ...
                for (let j = 0; j < wordParts.length; j++) {
                    ...
                    if (j === wordParts.length - 1)
                        // La última parte se puede combinar con más texto
                        currentLine = wordParts[j] + " "; // Las partes intermedias van solas en su línea
                    else lines.push(wordParts[j]);
                }
                continue;
            }
    ...
    if (authorMetrics.width > maxWidth) {
        ...
        const authorParts = breakLongWord(
            author,
            maxWidth - ctx.measureText("— ").width,
        );
...
async function drawMinimalistCanvas(message, cita, imagen, autor) {
    ...
    const profileCanvas = await createProfileImageWithGradient(
        imagen,
        canvasHeight,
    );

    // Crear texto
    const textCanvas = drawMinimalistText(
        cita,
        autor,
        canvasWidth - 300,
        canvasHeight,
...
function selectRandomStyle() {
    const random = Math.random(); // Puede ser exactamente 0.5
    let cumulativeWeight = 0;

    for (const [style, weight] of Object.entries(STYLE_WEIGHTS)) {
        cumulativeWeight += weight;
        if (random <= cumulativeWeight) return style; // <= con 0.5 + 0.5 = siempre retorna en primera iteración si random <= 0.5
    }
    return Object.keys(STYLE_WEIGHTS)[0]; // Fallback que llama ORIGINAL
}
    ...
    try {
        ...
        switch (selectedStyle) {
            case QUOTE_STYLES.ORIGINAL:
                return await drawCombinedCanvas(
                    message,
                    processedText,
                    imagen,
                    autor,
            ...
            case QUOTE_STYLES.MINIMALIST:
                return await drawMinimalistCanvas(
                    message,
                    processedText,
                    imagen,
                    autor,
            ...
            default:
                console.warn(
                    `Estilo desconocido: ${selectedStyle}, usando original`,
                );
                return await drawCombinedCanvas(
                    message,
                    processedText,
    ...
    run: async (client, message, args) => {
        ...
        if (args.length == 0 && !message.reference)
            return message.reply(
                "Tienes que poner una frase o responder a un mensaje, pendejo",
            );

        let autor,
            cita,
            imagen = `https://cdn.discordapp.com/avatars/`;
        ...
        if (message.reference) {
            ...
            if (frase.length == 0)
                return message.reply(
                    "Nambre wey, que quieres que cite si no escribió ni vrga",
                );
